...
>>>>>>> origin/master
	...
	public static List<Ticket> getAllManagersTickets(int mID) {
		...
		try {
			...
			stmt = conn.createStatement();
			String sql;
			sql = "SELECT DISTINCT Ticket.TicketID, Ticket.UserID, Ticket.EngineerID, Ticket.IssueID, Ticket.TimeEntered, Ticket.StateFlag, Ticket.ProblemDescription, Ticket.Screenshot, Ticket.CompletedTime, Ticket.NoOfReferrals, Ticket.RefferalHistory, Ticket.Priority FROM Engineer , Ticket , Manager WHERE Ticket.EngineerID = Engineer.EngineerID AND Engineer.ManagerID =" + mID;
...
<<<<<<< HEAD
			//Ticket t;
			String sql;
			sql = "SELECT * FROM Engineer, Person WHERE Engineer.PersonID=Person.PersonID AND Engineer.EngineerID=" + eID;
			ResultSet rs = stmt.executeQuery(sql);
			rs.first();
			Engineer e = new Engineer(rs.getInt("EngineerID"), rs.getInt("ExpertiseID"), rs.getInt("Availability"), rs.getInt("ManagerID"), rs.getInt("PersonID"), rs.getString("Title"), rs.getString("Forename"), rs.getString("Surname"), rs.getString("Email"), rs.getString("PhoneNumber"), rs.getString("Password"), rs.getString("Salt"));
			rs.close();
			stmt.close();
			conn.close();
			return e;
